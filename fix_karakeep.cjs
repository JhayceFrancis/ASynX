...
// Replace all occurrences of KarakeepLogo with a clean single copy
const match = content.match(/export const KarakeepLogo[^]+?(?=export const|$)/g);
if (match && match.length > 0) {
  content = content.replace(/export const KarakeepLogo[\s\S]*?(?=export const|$)/g, '');
