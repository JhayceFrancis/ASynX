...
def extract_inner(lines_list, name):
    ...
    # We remove the last `}` and the last `</div>`
...
# The backup end: `            </div>\n      }\n          )}\n        </div>\n`
library_raw = "".join(lines[805 : 1394])
library_inner = library_raw.split("{renderCustomizer('library')}")[1]
# For library, we remove the `}` that corresponds to layout.some, which was right before `)}`
