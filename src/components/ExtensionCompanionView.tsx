    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-cyan-950/60 via-slate-900 to-indigo-950/60 border border-cyan-500/30 rounded-3xl p-6 shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-bold text-slate-100">AniSync Matrix Browser Plugin Companion</h2>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-slate-950 border border-slate-700 rounded-3xl p-4 shadow-2xl space-y-4 max-w-sm mx-auto">
            {/* Window Header */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-bold text-slate-100">AniSync Matrix Helper</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="py-2 bg-slate-900 hover:bg-slate-800 text-slate-200 rounded-xl text-xs font-semibold border border-slate-700 transition cursor-pointer flex items-center justify-center space-x-1"
              >
                {state.overlayVisible ? <EyeOff className="w-3.5 h-3.5 text-slate-400" /> : <Eye className="w-3.5 h-3.5 text-cyan-400" />}
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] text-slate-400 font-medium">Select Streaming Portal</label>
                <select
                  value={selectedSite}
              ...
              <div>
                <label className="text-[11px] text-slate-400 font-medium">Playback Watch Progress (%): {testProgress}%</label>
                <input
                  type="range"
                  min="0"
                  max="100"
          ...
          <div className="relative rounded-3xl overflow-hidden border border-slate-800 shadow-2xl bg-black aspect-video flex items-center justify-center group">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-slate-950/60 flex items-center justify-center">
              <div className="w-16 h-16 rounded-full bg-slate-900/80 text-white flex items-center justify-center border border-slate-700 shadow-2xl">
                <Play className="w-8 h-8 text-cyan-400 ml-1" />
              </div>
            </div>

            {/* Video Player Header Overlay */}
            <div className="absolute top-4 left-4 right-4 flex items-center justify-between text-xs text-white bg-slate-950/80 px-4 py-2 rounded-2xl border border-slate-800/80 backdrop-blur-md">
              <span className="font-bold flex items-center space-x-2">
                <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse" />
                <span>Streaming on {selectedSite}</span>
              </span>
              <span className="text-slate-300 font-mono">1080p Full HD</span>
            </div>

            {/* Plugin On-Screen Corner HUD Widget Overlay (if visible) */}
            {state.overlayVisible && (
              <div className="absolute bottom-16 right-4 bg-slate-950/90 border border-indigo-500/40 rounded-2xl p-3 shadow-2xl backdrop-blur-md max-w-xs space-y-1.5 animate-fade-in">
                <div className="flex items-center justify-between text-[11px]">
                ...
                <p className="text-xs font-bold text-slate-100 line-clamp-1">{state.currentMedia?.title}</p>
                <div className="flex items-center justify-between text-[10px] text-slate-400">
