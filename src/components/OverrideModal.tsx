        ...
        <div className="flex items-start justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center space-x-3">
            <img
              src={item.coverImage}
              alt={item.title}
              className="w-12 h-16 object-cover rounded-xl shadow border border-slate-800"
            />
            <div>
              ...
              <h3 className="text-base font-bold text-slate-100 line-clamp-1 mt-0.5">{item.title}</h3>
              <p className="text-xs text-slate-400">{item.mediaType.toUpperCase()} • Total {item.totalEpisodes} episodes</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 text-slate-400 hover:text-white bg-slate-800 rounded-xl hover:bg-slate-700 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="font-semibold text-slate-300">Set Episode Progress</label>
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100 font-bold focus:outline-none focus:border-indigo-500 text-sm"
            />
          </div>

          {/* Target Status */}
          <div>
            <label className="font-semibold text-slate-300 block mb-1">Set Watch Status</label>
            <select
              value={targetStatus}
          ...
          {/* Rating Score */}
          <div>
            <label className="font-semibold text-slate-300 block mb-1">Rating Score (1 - 10)</label>
            <input
              type="number"
              min="1"
              max="10"
          ...
          <div>
            <label className="font-semibold text-slate-300 block mb-2">Apply Override to Platforms</label>
          ...
          <div className="pt-3 flex items-center space-x-2">
            <button
              type="button"
              onClick={onClose}
              className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-bold transition cursor-pointer"
            >
              Cancel
            </button>
            <button
